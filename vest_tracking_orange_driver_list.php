...
echo "<h1>Vest and Vehicle quick assignment status (readonly)</h1>";
	...
	$sql_drivers_assigned_orange = "select * from VestStatusView where VestStatusKey=1 and RouteKey=1 order by Modified";
        ...
        $sql_assign_car_res_orange = "select * from CarStatusView where RouteKey =1 order by CarStatusKey,Modified";
        $assign_car_res_orange = $conn->query($sql_assign_car_res_orange);
	...
	echo "<h2>Cars that are Currently Assigned</h2>";
        ...
        if ($assign_car_res_orange->num_rows > 0) {
                while($row = $assign_car_res_orange->fetch_assoc()) {
                        echo"<tr>";
